#ifndef FILE_LIST_STRING_H
#define FILE_LIST_STRING_H

#include "dependencies.h"
#include "NodeString.h"
#include "FileList.h"

template <> class FileList<std::string> {
public:
	// Конструктор файлового списка по умолчанию и копирования запрещены, т.к.
	// файл должен иметь имя и быть открыт только единожды
	FileList() = delete;
	FileList(const FileList<std::string>& copy) = delete;
	...
	void insert(const std::string& _data) {
		Node<std::string> tail;
		int64_t pos;
	...
	void insert(const std::string& _data, uint32_t index) {
		Node<std::string> tail;
		int64_t pos;
	...
	void insort(const std::string& data) {
		insert(data);
		sort();
	}
	...
	int64_t find(const std::string& _data) {
		Node<std::string> tail;
		int64_t pos;
...
private:
	...
	void swapNodes(Node<std::string>& left, Node<std::string>& right,
